...
if (isset($_POST['add_player'])) {
    ...
    $sql = "INSERT INTO joueurs (nom, prenom, numero_maillot, role, categorie_id, type_sport_id, image) 
            VALUES ('$nom', '$prenom', '$numero_maillot', '$role', '$id_categorie', 1, '$image_path')";
...
if (isset($_POST['edit_player'])) {
    ...
    $sql = "UPDATE joueurs SET nom='$nom', prenom='$prenom', numero_maillot='$numero_maillot', 
            role='$role', categorie_id='$id_categorie', image='$image_path' WHERE id='$id'";
    if ($conne->query($sql) !== TRUE) {
        echo "Erreur : " . $sql . "<br>" . $conne->error;
    }
}
...
// Récupérer la liste des joueurs de type "كرة القدم" par catégorie avec les noms des catégories et des types de sport
$sql_joueurs = "SELECT joueurs.*, categories.nom_categorie AS nom_categorie, types_photos.type AS type_sport 
                ...
                WHERE types_photos.type = 'كرة القدم'";
...
while ($row = $result_joueurs->fetch_assoc()) {
    $players_by_category[$row['nom_categorie']][] = $row;
}
...
<aside id="sidebar" class="js-sidebar">
    ...
    <div class="main">
        ...
        <div class="container">
            ...
            <?php foreach ($players_by_category as $category => $players): ?>
                                            ...
                                            <form action="" method="post" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="id_categorie" required>
                                                        <?php
                                                        $result_categories->data_seek(0); // Reset the result pointer to the beginning
                                                        while ($categorie = $result_categories->fetch_assoc()) {
                                                            $selected = ($categorie['id'] == $row['categorie_id']) ? 'selected' : '';
