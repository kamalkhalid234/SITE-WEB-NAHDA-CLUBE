...
// Récupérer les categories de sports disponibles
$sql_types = "SELECT DISTINCT nom_categorie FROM categories";
$result_types = $conne->query($sql_types);
$types = array();
while($row_type = $result_types->fetch_assoc()) {
    $types[] = $row_type['nom_categorie'];
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['add_match'])) {
        ...
        if (isset($_FILES['team1_logo']['name']) && isset($_FILES['team2_logo']['name']) && $_FILES['team1_logo']['error'] === 0 && $_FILES['team2_logo']['error'] === 0) {
            ...
            $type_sport_id = 1; // Store the constant value in a variable
            ...
            $sql = "INSERT INTO matches (type_sport_id, team1_logo, team2_logo, score, date, time, location, categorie_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conne->prepare($sql);
            $stmt->bind_param('issssssi', $type_sport_id, $team1_logo_path, $team2_logo_path, $score, $date, $time, $location, $categorie_id);
    ...
    if (isset($_POST['edit_match'])) {
        ...
        $time = $conne->real_escape_string($_POST['time']);
        $location = $conne->real_escape_string($_POST['location']);
        $categorie_id = $conne->real_escape_string($_POST['categorie_id']);
        ...
        $sql = "UPDATE matches SET team1_logo=?, team2_logo=?, score=?, date=?, time=?, location=?, categorie_id=? WHERE id=?";
        $stmt = $conne->prepare($sql);
        $stmt->bind_param('ssssssii', $team1_logo_path, $team2_logo_path, $score, $date, $time, $location, $categorie_id, $id);
...
<aside id="sidebar" class="js-sidebar">
        ...
        <div class="container mx-auto p-4">
            <h2 class="text-2xl font-bold mb-4">Liste des matchs par type</h2>
            ...
            <?php foreach ($types as $type): ?>
                <div class="mb-6">
                    <h3 class="text-xl font-bold mb-2"><?php echo $type; ?></h3>
                    <table class="w-full bg-white shadow-lg">
                        ...
                        <tbody>
                            ...
                            $sql_matches = "SELECT * FROM matches WHERE categorie_id IN (SELECT id FROM categories WHERE nom_categorie=?)";
                ...
                <form id="addMatchForm" method="post" action="matche_foot.php" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="categorie_id" name="categorie_id">
                            <?php $sql_cat = "SELECT * FROM categories"; $result = $conne->query($sql_cat) ; while ($row = $result->fetch_assoc()) {?>
